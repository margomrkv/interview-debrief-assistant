...
- **Размеченный датасет (EvalDataset)** — коллекция `AssessmentItem` с человеческой разметкой; используется для автоматической оценки качества (§7, E4-5).
...
| Кейс                              | CV | JD | Transcript | Feedback | Что делает система | MVP |
|-----------------------------------|----|----|------------|----------|--------------------|-----|
| Состояние до отклика              | ✓ | — | — | — | общие рекомендации (сценарий S1) | fallback |
| Новая заявка, до интервью         | ✓ | ✓ | — | — | рекомендации к подготовке (E4-1, E4-2) | post-MVP (S2) |
| Mock Карпова с YouTube            | — | — | ✓ | ✓ | агрегирует в корпус (E3-1), реконструирует псевдо-JD (E3-2) | ✓ MVP (S3) |
| Anton: ApprovalMax (без feedback) | ✓ | ✓ | ✓ | — | разбор без блока verdict (E4-4) | ✓ MVP (S4) |
| Anton: интервью Avito             | ✓ | ✓ | ✓ | ✓ | полный отчёт (E4-4) | ✓ MVP (S4) |
...
Замечание про изоляцию Eval-вызова: требование ментора («дело не в том, чтобы это были разные архитектуры, а чтобы контекст не шарили») — это **операционная** изоляция LLM-вызова (отдельный промпт, отдельное окно контекста), а не структурная изоляция артефактов. Артефакты `AssessmentItem`/`EvalDataset` остаются в KB; операционные требования к их использованию — в E4-5 (§7).
...
classDiagram
    ...
    EvalDataset "1" o-- "*" AssessmentItem : contains
```

Корпус — источник всего: рубрики, требования и Eval-айтемы — производные. Все стрелки замкнуты внутри Knowledge Base. Operational-аспект использования `EvalDataset` (отдельная дешёвая модель, изолированный контекст) — в E4-5 (§7), не в концептуальной модели.
...
Сценарии отличаются заполненностью матрицы артефактов и тем, какие группы (MF / KB) задействованы. В колонке «Наш кейс» указано, у кого из команды этот сценарий уже встречается. Колонка **MVP** фиксирует решение по итогам встречи с ментором 2026-04-30: фокус — S4 (ядро) и S3 (без него KB пустой), S2 — после чекпоинта 14.05.

| ID | Группы | Что есть на входе | Что хочет пользователь | Наш кейс                                  | MVP |
|----|--------|-------------------|-------------------------|-------------------------------------------|-----|
| **S1** | MF | Только профиль (CV + ценности + background) | Общие рекомендации по поиску, опираясь на знания агента | Мы до начала отклика                      | fallback (если KB пуст) |
| **S2** | MF + KB | Профиль + список вакансий | Ранжирование вакансий, акценты для отклика, упражнение «карьерный шкаф» | Мы при выборе, куда откликаться           | post-MVP |
| **S3** | KB | Корпус mock-интервью (Transcript + Feedback) без своего CV/JD | Эксплораторный анализ: типовые вопросы, рубрики, критерии оценки | Анализ mock-интервью Карпова и YouTube    | ✓ MVP (наполняет KB) |
| **S4** | MF + KB | Полный набор: профиль + JD + Transcript + Feedback | Структурированный отчёт по конкретному интервью с цитатами | Anton: интервью Avito, ApprovalMax и т.д. | ✓ MVP (ядро) |
...
**E2-3 «Разметочный датасет».** Как разработчик, я хочу вести `EvalDataset` (Google Sheets с `AssessmentItem`), чтобы автоматическая оценка качества (E4-5) опиралась на воспроизводимую человеческую разметку.
...
**E4-5 «Контроль качества».** Как разработчик, я хочу видеть автоматическую оценку качества `Recommendation` и `AssessmentItem`, чтобы понимать, не деградирует ли система между итерациями промпта (фидбэк ментора 2026-04-30: пользователю эта оценка не нужна и не показывается).
...
- [ ] **операционная изоляция вызова** (mentor: «дело не в том, чтобы это были разные архитектуры, а чтобы контекст не шарили»): отдельный (дешёвый) вызов LLM (Haiku / локальная Gemma 27B), собственный промпт, отдельное окно контекста — не разделяет state с основным пайплайном E4-4
...
**E4-6 «Структура рекомендаций».** Как кандидат, я хочу видеть рекомендации сгруппированными и со ссылками на источник, чтобы понимать, на основании чего они выданы.
