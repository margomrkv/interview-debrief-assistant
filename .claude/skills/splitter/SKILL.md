...
- **Не читать**, **не копировать**, **не мержить** и **не «исправлять точечно»** файлы предыдущих версий:
  - `splitter_output/.../{basename}.v(N-1).qa-split.json` (и любые `v1`, `v2`, … кроме **текущей** `vN` из шага 1);
...
Когда YouTube-видео содержит несколько кандидатов, каждый обрабатывается **отдельно**: своя папка в `transcripts/`, свой прогон сплиттера, свой `splitter_output/`.
