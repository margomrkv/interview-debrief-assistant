...
**Канон именования и дерево каталогов:** `scripts/transcript_downloader/NAMING.md` (зеркало: `transcripts/README.md`).  
...
| `bucket=<path>` | video | под `transcripts/`, напр. `mock-interviews/karpov` |
...
- **Single:** `wrote: transcripts/<bucket>/<slug>-<YYYY-MM-DD>/`
...
```text
transcripts/<bucket>/<slug>-<YYYY-MM-DD>/
```
...
- `transcripts/aliases.yaml` — история переименований
