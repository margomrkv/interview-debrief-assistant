...
> **Актуальный skill:** `.claude/skills/splitter/` — см. **`SKILL.md`**. Пайплайн по папкам `step1-prepare/` … `step5-validate-llm/`; артефакты в **`data/knowledgebase/splitted/`** (`*.qa-split.llm-input.vN.txt`, `*.qa-split.vN.json`, …). Ниже — устаревшее описание `run-splitter`.
...
**Обязательный аргумент:** путь к папке интервью (от корня репо), например `data/knowledgebase/raw/mock-interviews/avito/product-analyst-middle-avito-avito-2024-04-04`.
...
```
/run-splitter data/knowledgebase/raw/mock-interviews/avito/product-analyst-middle-avito-avito-2024-04-04
/run-splitter data/knowledgebase/raw/mock-interviews/karpov/data-analyst-junior-karpov-sasha-2021-09-13 version=2
/run-splitter data/knowledgebase/raw/mock-interviews/karpov/data-scientist-junior-karpov-2022-03-30 section_config=.claude/skills/run-splitter/config/section_topic_map.example.json
```
