...
Пользователи первой волны — сама команда проекта: инструмент мы делаем в первую очередь под себя, поэтому собственные кейсы — основной источник требований и тестовых данных. Архитектура должна допускать обобщение на «внешнего кандидата» позже.
...
- **Assessment and Recommendations** — основной модуль оценка и рекомендации кандидату по подготовке и развитию, а также структурированный отчёт о соответствии профиля кандидата требованиям вакансии.  Модуль запрашивает необходимые данные из Candidate Context, может как опираться на Knowledge Base, так и работать без него
...
- только Assessment and Recommendations на знаниях агента (полезно);
- Assessment and Recommendations на основе Knowledge Base — Evaluator работает по промпту, обученному на ground-truth-корпусе (Mocked QA + reference score), accuracy измерена на отложенном test-сабсете (хорошо/отлично).

### 2.2. Helicopter view

Высокоуровневая карта: является активным модулем Assessment and Recommendations и он использует два остальных (Candidate Context, Knowledge Base)
...
flowchart LR
    ...
    subgraph AR["Assessment and Recommendations<br/>(выход системы)"]
        direction TB
...
|--------|-----------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|--------------------|------------------------------------------------------------------------------------------------------|
| **Candidate Context** | Контекст по кандидату в разрезе рынка найма                                                                                                   | Сам кандидат через события  | Высокая — каждое событие | Профиль, заявка в Company A, раунд 2 поведенческого интервью с транскриптом                             |
...
| **Assessment and Recommendations** | Выход системы: транскрипт → per-question оценка + общий verdict                                                                               | оценка и формирование отчёта  | Производный: пересчитывается на каждый раунд | `AssessmentItem[]` по раунду + `AlignmentReport` (`verdict ∈ {HIRE, NO_HIRE}` + `p_hire ∈ [0, 100]`) |

### 2.3. Prerequisite: ручной ввод кейса
...
### 2.5. Высокоуровневая архитектура
...
Связка S3 → S4: артефакт `EvaluatorPrompt` (§3.1 Generic). S3 — самостоятельный entry-point в MVP (skill или режим в feedback-report); общая часть Splitter переиспользуется обоими.

Общая часть документирована в [[arch_pipeline]]; AR-pipeline и KB-pipeline (train-loop) — в [[arch_agents]].
...
### 3.2. Candidate Context (информация о конкретном кандидате и его пути по рынку найма)
- **Профиль кандидата** — расширенный контекст: ценности, принципы, background, soft/hard skills. На уровне модели §4.3 представлен парой `Candidate` (владелец, ценности/принципы/background) + `ExperienceProfile` (skills/achievements); в §4.1/§4.2 субъект `AlignmentReport` — `ExperienceProfile` (то, что сравнивается с JD).
...
Запрашиваемый контекст из CC: `InterviewTranscript` (обязательно), опц. `JobDescription` / `ExperienceProfile` / `Feedback` (см. §3.5 матрица заполненности).
...
### 3.6. Критерии работы assessor-ов

[[assessors]] описывает критерии, по которым human или AI assessor выставляет оценки в `AssessmentItem`.
...
- **`KnowledgeBase`** package — KB-specific: `MockedQA[]` (golden ground-truth корпус).
- **`AssessmentAndRecommendations`** package — минимальный AR-output: `AlignmentReport` (verdict + p_hire + items). Расширения (AssessmentTopic / Recommendation / topic_assessments / strengths_summary / gaps_summary) вынесены в [[requirements_postponed]] §5.
- **Generic** (вне package'ов) — `QA`, `AssessmentItem`, `Score`, `InterviewTranscript`, `Feedback`, `EvaluatorPrompt`: используются и KB (наполняется ими в режиме S3, см. §5), и AR (rollup'ятся в `AlignmentReport` в режиме S4). Согласуется с §2.3 (общий pipeline).
- **CC refs** (вне package'ов, помечены `(CC)`) — `ExperienceProfile`, `JobDescription`: внешние классы из §4.3, к которым AR обращается за контекстом; внутрь Generic / KB не приходят (инвариант §2).

Стрелки на диаграмме — **dataflow** (данные текут от источника к потребителю). «Активная сторона AR» (AR запрашивает CC и KB), описанная словесно в §2, — это dependency, инверсия dataflow.
...
classDiagram
    ...
    namespace AssessmentAndRecommendations {
    ...
    EvaluatorPrompt "1" ..> "*" AssessmentItem : produces
...
### 4.2. Assessment and Recommendations (rollup-flow view)
...
Стрелки — **dataflow** (как в §4.1). Семантика «AR активный, запрашивает CC/KB» — словесная (§2), на диаграмме это инверсия dataflow.

```mermaid
flowchart LR
    %% Входы (запрашиваются AR'ом)
    CC_T["InterviewTranscript<br/>(обязательно)"]
    CC_J["JobDescription<br/>(CC, опц.)"]
    CC_P["ExperienceProfile<br/>(CC, обязательно)"]
    CC_F["Feedback<br/>(опц.)"]
    KB_P["EvaluatorPrompt<br/>(обучен в S3)"]

    %% Rollup-чейн внутри AR
    subgraph AR["AssessmentAndRecommendations"]
        direction TB
        QA["QA"]
        AI["AssessmentItem"]
...
Обязательная привязка `AlignmentReport` к `ExperienceProfile` (сплошная стрелка от CC_P) — без `subject` отчёт некому адресовать. Остальные CC-входы опциональны по §3.5. `EvaluatorPrompt` приходит из KB (выход S3-train, см. §2.3).
...
В MVP — два сценария: **S3** (обучение промпта оценки) и **S4** (собственно модуль рекомендаций на базе промпта). 
...
Общая часть — [[arch_pipeline]]; AR-pipeline и KB-pipeline (train-loop) — [[arch_agents]] §4.
...
- [[arch_agents]] — `md/arch_agents.md` — AR-pipeline и KB-pipeline (train-loop): включает общую часть [[arch_pipeline]] плюс AR-specific S4-Aggregator и KB-specific S3-PromptTuner
